...
const WalletCard = () => {
    ...
    const addCard = async () => {
        ...
        try {
            ...
            if (resp) {
                const updateCard = [...card, formInput].slice(0, 1);
                setCard(updateCard);
                setFormInput({
                    cardNum: '',
                    expiryDate: '',
    ...
    const addAddress = async () => {
        ...
        try {
            ...
            if (resp) {
                const updateAddress = [...address, formAddress].slice(0, 3);
        ...
        <div className='grid gap-6 '>
            ...
            <AddAddressModal showAddAddressModal={showAddAddressModal} onClose={() => setShowAddAddressModal(false)}>
                <div className='grid gap-4'>
                    ...
                    >
                        {loading ? <Oval visible={true} height="20" width="20" color="#ffff" ariaLabel="oval-loading" wrapperStyle={{}} wrapperClass=""/> : 'Add Address'}
                    </button>
                </div>
            </AddAddressModal>
            ...
            <AddCardModal showAddCardModal={showAddCardModal} onClose={() => setShowAddCardModal(false)}>
                <div className='grid gap-4'>
                    ...
                    <button
                        ...
                        className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-[--bg-color] bg-[--button-color]"
                    >
                        {loading ? <Oval visible={true} height="20" width="20" color="#ffff" ariaLabel="oval-loading" wrapperStyle={{}} wrapperClass=""/> : 'Add Card'}
                    </button>
                </div>
            </AddCardModal>
        </div>
