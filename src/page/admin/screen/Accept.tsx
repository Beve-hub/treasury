...
const Accept = ({ toggleIcon,  onClose, children }:AcceptProps) => {
    return (
        <div onClick={onClose} className={`fixed inset-0 flex justify-center items-center p-4  ${toggleIcon ? "visible bg-black/20" : "invisible"}`}>
          <div className={`bg-white p-4 h-[10em] w-[20rem] grid items-center rounded-md ${toggleIcon ? 'scale-100 opacity-100' : 'scale-110 opacity-0'}`} 
